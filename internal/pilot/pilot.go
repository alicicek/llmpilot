...
func NotAvailable(feature string) error {
	return fmt.Errorf("%s is part of llmpilot Pro, and this build was compiled without it — the official app at https://llmpilot.dev includes Pro; switching, statusline, cockpit, and analytics all keep working here", feature)
}
...
func NotLicensed(feature string) error {
	return fmt.Errorf("%s is paused because this app has no current llmpilot Pro entitlement — open Settings → License to activate or recover it", feature)
}
