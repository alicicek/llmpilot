...
function colorFor(kind: Kind): string {
  if (kind === "rotation") return "var(--graydot)";
    ...
    <div className="not-prose">
      ...
      >
        ...
        <div className="flex items-center gap-2 border-b border-hair px-4 py-2.5">
          ...
          <span className="h-3 w-3 rounded-full" style={{ background: "var(--graydot)" }} />
          <span className="h-3 w-3 rounded-full" style={{ background: "var(--graydot)" }} />
          <span className="flex-1 text-center text-[12px] text-ter">statusline</span>
          <span className="text-[12px] text-ter" aria-hidden="true">
            {MODE_LABELS[mode]}
          </span>
        </div>

        {/* the draggable segment row */}
        <Reorder.Group
          axis="x"
          values={items}
          onReorder={setItems}
          ...
          {items.map((seg) => {
            ...
            return (
              <Reorder.Item
                ...
                className="list-none cursor-grab touch-none rounded-sm border border-hair bg-win px-2.5 py-1.5 leading-none"
                style={{ color: colorFor(seg.kind) }}
                {...(isUsage
                  ? {
                      role: "button",
                      tabIndex: 0,
                      "aria-label": `${seg.id} usage, mode ${MODE_LABELS[mode]} — activate to change mode`,
                      onClick: () => {
