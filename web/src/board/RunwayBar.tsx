...
export default function RunwayBar({ bucket, refIso, stale }: RunwayBarProps) {
  ...
  return (
    <div className="mt-[5px] flex items-center gap-[7px]">
      ...
      {/* Track is the rail token (the mock's spec — hairsoft was drift). At
          warn and above it also carries a terminus tick under the fill: a
          rounded fill end looks identical to a full bar, so the tick marks
          where the track really ends and the gap before it IS the headroom
          (covered only at 100%). Below warn the gap speaks for itself and a
          lone tick on an empty track just reads as a speck. */}
